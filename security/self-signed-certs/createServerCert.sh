...
cd CA/intermediate
openssl req -config openssl.cnf -key ../../server/${SERVER_NAME}.key.pem -new -sha256 -out ../../server/${SERVER_NAME}.csr.pem

echo "Generating certificate file"
openssl ca -config openssl.cnf -extensions server_cert -days 730 -notext -md sha256 -in ../../server/${SERVER_NAME}.csr.pem -out ../../server/${SERVER_NAME}.cert.pem
